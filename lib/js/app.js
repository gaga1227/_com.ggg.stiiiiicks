...
window.App = (function (app) {
	...
	App.utils = {
		...
		popMsg: function (options) {
				...
				onComplete: _.bind(function () {
					...
					if (options.duration === 0) {
						...
						$modal.one('click', function () {
							TweenLite.to($overlay[0], App.view.animSpeed.norm / 2, overlayPropsOut);
			...
			TweenLite.to($overlay[0], App.view.animSpeed.norm / 2, overlayPropsIn);
...
window.App = (function (app) {
	...
	App.view = {
		...
		updateDifficulty: function () {
			...
			// first align
			if (App.data.game.sticksAligned === this.sticksData.length) {
			...
			// keep difficulty
			else if (App.data.game.sticksAligned < 30) {
				App.data.game.scrambleMoves = 1;
			}
			// advance difficulty

...
window.App = (function (app) {
	...
	App.view = {
		...
		positionSticksByStick: function (byStickIndex, repeat) {
			...
			// pick random stick except for root stick
			var primaryStickIndex = Math.ceil(Math.random() * (this.sticksData.length - 1));

			// calculate stick positions
			while (repeat--) {
				this.calculateStickPositionsBy(byStickData, primaryStickIndex);
			}
		},
		...
		calculateStickPositionsBy: function (byStickData, primaryStickIndex) {
			// get level data
			var levelData = App.data.levels[App.data.levels.defaultLevel];
			...
			// byStickData deter level
			byStickData.deterLevel = Math.abs(byStickData.id - primaryStickIndex) + 1;
			var byStickPosShiftAmount = this.maxMoveSegments / Math.pow(deterRatioDivider, byStickData.deterLevel);

			// get swipe direction
			var swipeDirection = this.getSwipeDirection(byStickData.pos, byStickPosShiftAmount);

			// get by stick position before swipe
			var byStickOldPos = (swipeDirection == 'left') ? byStickData.pos + byStickPosShiftAmount : byStickData.pos - byStickPosShiftAmount;
			if (byStickOldPos < 0) {
				byStickOldPos = Math.abs(byStickOldPos);
			}
			if (byStickOldPos > this.maxMoveSegments) {
				byStickOldPos = this.maxMoveSegments * 2 - byStickOldPos;
			}

			console.group('[view.calculateStickPositionsBy]');
			console.log('primaryStickIndex: ', primaryStickIndex);
			console.log('byStickIndex: ', byStickData.id);
			console.log('byStickPos: ', byStickData.pos);
			console.log('swipeDirection: ', swipeDirection);
			console.log('byStickOldPos: ', byStickOldPos);
		...
		resetSticks: function () {
			...
			this.positionSticksByStick(rootStickIndex, 3);
			// render all sticks
			this.renderSticks();
		},

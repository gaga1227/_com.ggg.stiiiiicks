...
window.App = (function (app) {
	...
	App.view = {
				...
				if (this.hintData.moveHintIsActive) {
					...
					if (absMoveAmount <= offsetAmountLimit) {
						_.forEach(this.sticksData, _.bind(function (stickData) {
							...
							// animate primary stick hint
							if (deterLevel === 1) {
								this.animateStickActive(stickData.$el[0]);
							} else {
								this.animateStickInactive(stickData.$el[0]);
							}
						}, this));
					}
				}
		...
		updateGameStatus: function () {
			...
			// update status
			if (App.data.game.stiiiiicks < 0) {
				status = 'lost';
				console.log('[updateGameStatus]: ' + status);

				// trigger lost game dialogue
				this.toggleDialogue({
					title: 'Oops,<br>You have Lost!',
					actionLabel: 'Try Again',
					actionMethod: _.bind(function () {
						this.restartLevel();
						this.toggleDialogue();
					}, this)
				});
		...
		restartLevel: function () {
			// get current level
			var level = App.data.levels.currentLevel;

			// restart level
			this.initGame(App.data.levels[level]);

			// set selected level button
			var $btn = this.$settings.find('.btnLevel[data-level="' + level + '"]');
			if ($btn.length) {
				$btn
					.addClass('selected')
			...
			console.log('[restartLevel]: ' + level);
		},
		...
		initGame: function (levelData) {
			...
			this.updateScoreText(levelData.initialStiiiiicks);
		...
		// toggle game dialogue
		toggleDialogue: function (data) {
			// vars
			var levelData = App.data.levels[App.data.levels.currentLevel];
			var $title = this.$dialogue.find('.title');
			var $btn = this.$dialogue.find('.btnAction');
			...
			if (data) {
				...
				$btn
					...
					.off('click.button')
					.on('click.button', data.actionMethod);

				// update DOM layers
				if (this.$settings.hasClass('active')) {
					this.$settings.find('.btnSettings').trigger('click.button');
				}
				this.$game.addClass('fade');
				this.$dialogue
					.addClass(this.gui.levelClasses[levelData.id])
					.addClass('active');

				// add bg noise
				this.initNoise(this.$dialogue);
			}

			// if toggle off
			else {
				// reset dialogue content
				$title.text('');
				...
				// update DOM layers
				this.$game.removeClass('fade');
				this.$dialogue
					.removeClass(this.gui.levelClasses.join(' '))
					.removeClass('active');

				// remove bg noise
				this.$dialogue.removeAttr('style');
			}
		},

		// update score text
		updateScoreText: function (forceScore) {
			// score text elem
			...
			// apply force score
			if (!_.isUndefined(forceScore) && !_.isNaN(forceScore)) {
				App.data.game.stiiiiicks = Math.round(forceScore);
			}
		...
		// add bg noise
		initNoise: function ($target, options) {
			// exit
			if (_.isUndefined($target) || !$target.length) {
				return '[View.initNoise]: Invalid target';
			}
		...
		init: function () {
			...
			this.initNoise(this.$settings);
			this.initNoise(this.$gameBg);
			this.initSettings();

		...
		updateMoveHintOnTouch: function (e) {
			...
			if (e.type === 'touchstart') {
				...
				this.hintData.moveHintTimeout = setTimeout(_.bind(function () {
					this.hintData.moveHintIsActive = true;
				...
				if (this.hintData.moveHintIsActive) {
					...
					if (absMoveAmount <= offsetAmountLimit) {
						_.forEach(this.sticksData, _.bind(function (stickData) {
							...
							var hintOffsetAmount = moveAmount / Math.pow(deterRatioDivider, deterLevel);

							// calculate edge bounce amount if applicable
							var stickX = this.canvasData.movePositions[stickData.newpos];
							var hintOffsetX = stickX + hintOffsetAmount;
							if (hintOffsetX > this.canvasData.width) {
								hintOffsetAmount = this.canvasData.width - (hintOffsetX - this.canvasData.width);
								hintOffsetAmount -= stickX;
							} else if (hintOffsetX < 0) {
								hintOffsetAmount = 0 - hintOffsetX;
								hintOffsetAmount -= stickX;
							}

							// TODO: count in stick margin on edges

	...
	App.view = {
		...
		shrinkStick: function (idx) {
			if (idx == (this.sticksData.length - 1)) {
				return '[shrinkStick]: Cannot shrink last Stick!';
			}
			if (!this.sticksData[idx].$el.length) {
				return '[shrinkStick]: Invalid stick element!';
			}
			...
			var cssProps = {
				...
				onComplete: _.bind(function () {
					this.resetStickData(idx);
					this.shrinkStick(idx + 1);
				}, this)
			};

			// start animation
			TweenLite.to(stickEle, this.animSpeed.norm * 2, cssProps);
		},

		// resetStickData
		resetStickData: function (idx) {
			if (idx >= (this.sticksData.length - 1)) {
				return '[resetStickData]: Invalid stick index!';
			}

			// TODO: reset shrunk stick data to new random pos
			// ...
		},

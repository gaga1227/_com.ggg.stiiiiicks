window.App = (function(app) {
	...
	App.view = {
		...
		positionSticks: function(repeat) {
			...
			var primaryStickIndex = Math.floor(Math.random() * this.sticksData.length);
		...
		calculateStickPositions: function(primaryStickIndex, swipeDirection) {
			...
			_.forEach(this.sticksData, _.bind(function(stickData) {
				...
				var posShiftAmount = (this.maxMoveSegments / 2) / Math.pow(deterRatioDivider, stickData.deterLevel);

				if (swipeDirection == 'left') {
					console.log('old', stickData.newpos, 'left', posShiftAmount);
					// on swipe left
					...
					console.log('old', stickData.newpos, 'right', posShiftAmount);
					// on swipe right
					stickData.newpos = stickData.pos + posShiftAmount;
				...
				console.log('new', stickData.newpos);
			}, this));
		},
